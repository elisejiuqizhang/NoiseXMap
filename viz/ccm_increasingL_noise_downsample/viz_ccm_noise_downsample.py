...
viz_save_dir=os.path.join(root, 'outputs','viz', 'ccm_increasingL_noise_downsample')
            ...
            for noiseAddType in list_noiseAddTypes: # add, mult, both
                for noiseWhen in list_noiseWhen: # in, post
                    ...
                    for downsampleType in list_downsampleTypes: # average, decimation, subsample - each type is having its own plot along with the reference and the noise-noDownsample curves
                        noise_D_dir=os.path.join(noise_D_dir, downsampleType+'_')
                        ...
                        for downsampleFactor in list_downsampleFactors:
                            noise_D_factor_dir=os.path.join(noise_D_dir+str(downsampleFactor),f'tau{tau}_emd{emd}', noiseType, noiseWhen+'_'+noiseAddType+'_')
                            # load the .npy files in order of the noise levels
                            noise_D_avg_sc1=[]
                            noise_D_avg_sc2=[]
                            for noiseLevel in list_noiseLevels: # from 0.005, 0.01, 0.015, 0.02,..., 0.75
                        ...
                        ax.plot(list_noiseLevels, noise_noD_avg_sc1, 'r-', label=f'{noiseType}_{noiseAddType}_{noiseWhen}-no Downsampling')
                        ...
                        ax.plot(list_noiseLevels, noise_noD_avg_sc2, 'r-', label=f'{noiseType}_{noiseAddType}_{noiseWhen}-no Downsampling')
                        for i in range(len(list_downsampleFactors)):
