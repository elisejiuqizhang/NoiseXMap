...
output_dir=os.path.join(root, 'outputs', 'viz', 'filters', 'extendKalman')
...
def extended_kalman_filter(measurements, dt, process_noise_std, measurement_noise_std, noiseType=None, noiseAddType=None, noiseLevel=None):
    def jacobian_matrix(xyz, s=10, r=28, b=2.667):
        x, y, z = xyz
        J = np.array([[-s, s, 0],
                      [r - z, -1, -x],
                      [y, x, -b]])
...
dt = 0.01
filtered_states = extended_kalman_filter(data, dt, process_noise_std, measurement_noise_std)
